...
type Signer struct {
    ...
    Credentials generic_structs.ApiCredentials
...
func NewSigner(credentials generic_structs.ApiCredentials, options ...func(*Signer)) *Signer {
    v4 := &Signer{
...
type signingCtx struct {
    ...
    credValues         generic_structs.ApiCredentials
    isPresign          bool
...
//var SignRequestHandler = request.NamedHandler{  // TODO
...
func (ctx *signingCtx) build(disableHeaderHoisting bool) error {
    ...
    if ctx.isPresign {
        ...
        parts := []string{
            authHeaderPrefix + " Credential=" + ctx.credValues.Id + "/" + ctx.credentialString,
...
func (ctx *signingCtx) buildCredentialString() {
    ...
    if ctx.isPresign {
        ctx.Query.Set("X-Amz-Credential", ctx.credValues.Id+"/"+ctx.credentialString)
    }
}
...
func makeSha256Reader(reader io.ReadSeeker) []byte {
    hash := sha256.New()
    start, _ := reader.Seek(0, aws_signer_structs.SeekCurrent)
    defer reader.Seek(start, aws_signer_structs.SeekStart)
